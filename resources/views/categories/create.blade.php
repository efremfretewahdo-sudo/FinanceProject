    ...
    <div class="py-6 max-w-lg">
        <div class="flex items-center gap-4 mb-8">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500 mt-0.5">Create a new category to organize transactions</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <form method="POST" action="{{ route('categories.store') }}" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" placeholder="e.g. Groceries, Salary, Rent..." required class="w-full text-sm border-gray-200 rounded-lg px-3 py-2.5 focus:ring-indigo-500 focus:border-indigo-500 @error('name') border-red-300 @enderror">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <label class="flex items-center justify-center gap-2 p-3 border-2 rounded-xl cursor-pointer {{ old('type') === 'income' ? 'border-green-500 bg-green-50' : 'border-gray-200' }}">
                        ...
                        <label class="flex items-center justify-center gap-2 p-3 border-2 rounded-xl cursor-pointer {{ old('type', 'expense') === 'expense' ? 'border-red-500 bg-red-50' : 'border-gray-200' }}">
                            <input type="radio" name="type" value="expense" class="sr-only" {{ old('type', 'expense') === 'expense' ? 'checked' : '' }}>
                ...
                <div>
                    ...
                    <div class="flex items-center gap-3">
                        <input type="color" name="color" value="{{ old('color', '#6366f1') }}" class="w-10 h-10 rounded-lg border-gray-200 cursor-pointer p-0.5">
